...
import { ShieldCheck, Activity, Database, Users, TrendingUp, BarChart4, Settings, LayoutDashboard, Globe, Zap, Box, Anchor, Share2, Server, Repeat, AlertTriangle, Coffee, Compass, Milestone, Rocket, LineChart } from 'lucide-react';
      ...
      <div className="flex min-h-screen bg-slate-950 text-slate-100 font-sans">
        ...
        <aside className="w-72 bg-slate-900/40 backdrop-blur-3xl border-r border-slate-800 flex flex-col p-8 fixed h-full shadow-2xl">
          <div className="flex items-center gap-4 mb-12">
            ...
            <span className="text-xl font-black tracking-tight bg-gradient-to-r from-white to-slate-400 bg-clip-text text-transparent">DevEx IQ</span>
          </div>
          
          <nav className="flex-1 space-y-2">
            <NavItem to="/" icon={<LayoutDashboard size={20} />} label="Executive Summary" active />
            <NavItem to="/dora" icon={<Rocket size={20} />} label="DORA Plus" />
            <NavItem to="/flow" icon={<Activity size={20} />} label="Engineering Flow" />
            <NavItem to="/onboarding" icon={<Compass size={20} />} label="Onboarding Center" />
            <NavItem to="/adoption" icon={<Milestone size={20} />} label="Platform Adoption" />
            <NavItem to="/satisfaction" icon={<Users size={20} />} label="Sentiment Score" />
            <NavItem to="/benchmarks" icon={<LineChart size={20} />} label="Team Benchmarks" />
          </nav>
        ...
        <main className="flex-1 ml-72">
          <header className="h-20 border-b border-slate-800 flex items-center justify-between px-10 bg-slate-950/50 backdrop-blur-md sticky top-0 z-10">
            <div className="flex items-center gap-2 text-slate-400 text-sm font-medium uppercase tracking-widest">
              ...
              <span>/</span>
              <span className="text-white font-bold">Global Engineering Effectiveness</span>
            </div>
            <div className="flex items-center gap-4">
              <div className="text-right">
                <p className="text-sm font-bold text-white">Platform Architect</p>
                <p className="text-[10px] text-blue-400 uppercase tracking-widest font-black">Industrialized DevEx</p>
              </div>

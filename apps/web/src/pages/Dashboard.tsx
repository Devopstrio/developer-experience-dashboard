...
import { ShieldCheck, Activity, Database, TrendingUp, RefreshCcw, ExternalLink, Zap, Coffee, Compass, Milestone, Users } from 'lucide-react';

const doraTrend = [
  { name: 'Jan', velocity: 45 },
  { name: 'Feb', velocity: 52 },
  { name: 'Mar', velocity: 48 },
  { name: 'Apr', velocity: 64 },
  { name: 'May', velocity: 72 },
];

const adoptionByTool = [
  { name: 'Service Catalog', score: 92 },
  { name: 'Self-Service K8s', score: 74 },
  { name: 'CI Templates', score: 88 },
  { name: 'CLI Tooling', score: 65 },
];
    ...
    <div className="space-y-12 animate-fade-in">
      ...
      <div className="flex justify-between items-end border-b border-slate-800 pb-8">
        <div>
          ...
          <p className="text-slate-400 mt-3 text-xl max-w-2xl font-medium">Enterprise global engineering health, productivity benchmarks, and platform adoption risk hubs.</p>
        </div>
        <div className="flex gap-4">
          <button className="bg-slate-900 border border-slate-800 text-slate-300 px-6 py-3 rounded-2xl font-bold hover:bg-slate-800 transition flex items-center gap-2">
             <RefreshCcw size={18} /> Sync Tooling
          </button>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
        <StatCard title="DevEx Index" value="88.2%" change="+4.4%" icon={<Coffee className="text-blue-400" />} />
        <StatCard title="Onboarding" value="4.2d" change="-1.2d" icon={<Compass className="text-emerald-400" />} />
        <StatCard title="Flow Efficiency" value="64%" change="+8%" icon={<Activity className="text-blue-400" />} />
        <StatCard title="Sentiment" value="4.2/5" change="Optimal" icon={<Users className="text-amber-400" />} />
      </div>
        ...
        <div className="bg-slate-900/40 border border-slate-800 p-10 rounded-[3rem] backdrop-blur-3xl shadow-2xl relative overflow-hidden">
          <div className="flex items-center justify-between mb-10">
            <h2 className="text-2xl font-bold text-white flex items-center gap-3">
              ...
              Engineering Velocity Trend (DORA Plus)
            </h2>
            <div className="text-[10px] font-black uppercase text-blue-400 tracking-[0.2em] bg-blue-900/20 px-4 py-2 rounded-full border border-blue-800/50">Industrialized Flow</div>
          </div>
          <div className="h-[400px] w-full">
            <ResponsiveContainer width="100%" height="100%">
              <AreaChart data={doraTrend}>
                <defs>
        ...
        <div className="bg-slate-900/40 border border-slate-800 p-10 rounded-[3rem] backdrop-blur-3xl shadow-2xl relative overflow-hidden">
          <div className="flex items-center justify-between mb-10">
            <h2 className="text-2xl font-bold text-white flex items-center gap-3">
              ...
              Platform Adoption by Internal Tooling (%)
            </h2>
            ...
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={adoptionByTool}>
                ...
                <Bar dataKey="score" radius={[8, 8, 0, 0]} barSize={50}>
                  {adoptionByTool.map((entry, index) => (
      ...
      <div className="bg-slate-900/40 border border-slate-800 p-10 rounded-[3rem] backdrop-blur-3xl shadow-2xl">
        <h2 className="text-2xl font-bold mb-10 text-white flex items-center gap-3 text-blue-400 uppercase tracking-widest">
           <TrendingUp size={28} />
           High-Priority Team Effectiveness Insights
        </h2>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
           <ProjectCard name="Team Alpha" metrics="PR Latency: 14h" status="Healthy" score="92.4%" color="text-emerald-400" />
           <ProjectCard name="Platform Core" metrics="Adoption: 84%" status="Warning" score="72.0%" color="text-amber-400" />
           <ProjectCard name="Mobile App" metrics="Onboard: 3.5d" status="Healthy" score="88.0%" color="text-emerald-400" />
        </div>
      </div>
    </div>
  );
...
const ProjectCard = ({ name, metrics, status, score, color }: any) => (
  <div className="p-6 bg-slate-950/50 border border-slate-800 rounded-3xl hover:bg-slate-800/50 transition cursor-pointer group">
    ...
    <div className="flex justify-between items-center mt-8">
       <span className="text-[10px] font-black uppercase text-slate-500 tracking-widest italic">Signal: {metrics} | Score: {score}</span>
